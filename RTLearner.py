...
class RTLearner(object):
    ...
    def tree_recursive_func(self, data_x, data_y):
        ...
        while factor_index.shape[0] > 0:
            if factor_coeff_corr[factor_coeff_n][0] < 0 or factor_coeff_corr[factor_coeff_n][0] == np.nan:
                factor_coeff_corr[factor_coeff_n][0] = 0
            best_f = min(factor_coeff_corr[factor_coeff_n][0],factor_index.shape[0]-1)
            # print('factor_coeff_corr ', factor_coeff_corr)
            # print('best_f ', best_f)
            # print('data_x[:, int(best_f)] ', data_x[:, int(best_f)])
            split_val = np.median(data_x[:, int(best_f)])
            # split_val = np.average(data_x[:, int(best_f)])
